...
function getedgetocalcu(msgedgeid, gettingtype)
	_msgedgearray = msgedgearray
	_factorneighborsarray = factorneighborsarray
	_variableneighborsarray = variableneighborsarray
...
function bpinference(vs::Array{Variable,1}, fs::Array{Factor,1}, ts::Array{Float64,1})
	init(vs, fs, ts)
	...
	#println(facsnum)
	#println(edgesnum)
	#println(msgedgearray)
	#println(factorneighborsarray)
	#println(variableneighborsarray)

	println("----------beliefpropagation initialized----------")

	run()

	#println(msgedgearray)

	println("----------beliefpropagation message calculated----------")

	inference()

	println("----------beliefpropagation propability calculated----------")
end